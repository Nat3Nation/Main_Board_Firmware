...
ADE9000::ADE9000(PCA9671BS *expander, int id)
{
    _chipSelect_Pin = 25;
    m_flipCurr = false;
    expander_ = expander;
...
Output:-
*/
void ADE9000::SPI_Init(uint32_t SPI_speed , uint8_t chipSelect_Pin, uint8_t en_val)
{
  expander_->digital_write(chipSelect_Pin, HIGH);   //Set Chip select pin high
...
void ADE9000:: SPI_Write_16(uint16_t Address , uint16_t Data )
{
  ...
  expander_->digital_write(PCA_PIN_P16, en_val_); // Enable pin
  expander_->digital_write(_chipSelect_Pin, LOW);
  temp_address = ((Address << 4) & 0xFFF0); //shift address  to align with cmd packet
  SPI.transfer16(temp_address);
  SPI.transfer16(Data);
  
  expander_->digital_write(_chipSelect_Pin, HIGH);  
}
...
int32_t ADE9000::ReadConfigRegisterValue(){
    /*WRITE TO RUN REGISTER*/
  // Serial.println("Writing to run register...");
  // SPI_Write_16(RUN_REGISTER, ADE9000_RUN_ON);

  delay(200);
  /*Read Config Register*/
  Serial.print("Value read from the run register: ");
  int16_t test = (SPI_Read_16(RUN_REGISTER));
  Serial.println(test);
  Serial.print("Value read from the last16 register: ");
  int16_t test2 = (SPI_Read_16(LAST_DATA_16));
...
float ADE9000::calibrate_VRMS() {
  
  Serial.println();
  Serial.print("Calibrating the terminal ID = ");
  Serial.println(id_);

  // Reset the calibration registers
  SPI_Write_32(ADDR_AVGAIN, (int32_t) 0x0);  
  SPI_Write_32(ADDR_BVGAIN, (int32_t) 0x0);
  SPI_Write_32(ADDR_CVGAIN, (int32_t) 0x0);

  while(1) {
    String v_nom = getUserInput("Please indicate the desired nominal voltage (in V): ");
    V_nom_ = v_nom.toDouble();
    if (V_nom_ < 0.0) {
      Serial.println("Please enter an apporpriate nominal voltage");
    } else {
      break;
    }
  }

  // Wait for the to apply the nominal voltage
  flushInputBuffer();
  Serial.print("Please source the following voltage on all three channels: ");
  Serial.println(V_nom_);
  Serial.println();
  Serial.println("Press enter whenever ready.");
  while(Serial.available() == 0);

  // Calcualte V_fsp:
  float V_fsp = (V_TRANSFER*V_nom_)/0.7071;

  // Compute the expected reading
  V_expected_ = V_fsp * ADE9000_RMS_FULL_SCALE_CODES;

  // Read the voltages on the channels
  VoltageRMSRegs RMS_voltage; 
  ReadVoltageRMSRegs(&RMS_voltage);
  int32_t measured_A = RMS_voltage.VoltageRMSReg_A;
  int32_t measured_B = RMS_voltage.VoltageRMSReg_B;
  int32_t measured_C = RMS_voltage.VoltageRMSReg_C;

  // Compute the gains
  float gain_A = (float) V_expected_ / (float) measured_A;
  float gain_B = (float) V_expected_ / (float) measured_B;
  float gain_C = (float) V_expected_ / (float) measured_C;
  int32_t avgain = (gain_A - 1)*TWO_TO_27;
  int32_t bvgain = (gain_B - 1)*TWO_TO_27;
  int32_t cvgain = (gain_C - 1)*TWO_TO_27;

  // Write to the registers
  SPI_Write_32(ADDR_AVGAIN, (int32_t) avgain);  
  SPI_Write_32(ADDR_BVGAIN, (int32_t) bvgain);  
  SPI_Write_32(ADDR_CVGAIN, (int32_t) cvgain);  

  return V_expected_;
}

int32_t ADE9000::V_expected() {
  return V_expected_;
}
...
String ADE9000::generate_json() {
  VoltageRMSRegs RMS_voltage; 
  this->ReadVoltageRMSRegs(&RMS_voltage);
  // ade.ReadCurrentRMSRegs(&RMS_current);
  // Create a DynamicJsonDocument
  DynamicJsonDocument doc(2048);

  // Add data to the document
  doc["action"] = "energy_record";
  doc["ade_id"] = id_;
  // Create a nested object
  JsonObject payload = doc.createNestedObject("payload");
  float VA_MAG = (float) V_nom_/(V_expected_) * RMS_voltage.VoltageRMSReg_A;
  float VB_MAG = (float) V_nom_/(V_expected_) * RMS_voltage.VoltageRMSReg_B;
  float VC_MAG = (float) V_nom_/(V_expected_) * RMS_voltage.VoltageRMSReg_C;
  float fixed_current_val = 20.0;
  float fixed_sin = 0.866;
  float fixed_cos = 0.5;
  payload["VA_MAG"] = VA_MAG;
  payload["VB_MAG"] = VB_MAG;
  payload["VC_MAG"] = VC_MAG;
  payload["IA_MAG"] = fixed_current_val;
  payload["IB_MAG"] = fixed_current_val;
  payload["IC_MAG"] = fixed_current_val;
  payload["VA_ANG"] = 0.00;
  payload["VB_ANG"] = 120.00;
  payload["VC_ANG"] = 240.00;
  payload["IA_ANG"] = 60.00;
  payload["IB_ANG"] = 180.00;
  payload["IC_ANG"] = 300.00;
  payload["POW_FACTOR"] = fixed_cos;
  payload["POW_APPARENT"] = 1.732*VA_MAG*fixed_current_val;
  payload["POW_ACTIVE"] = 1.732*VB_MAG*fixed_current_val*fixed_cos;
  payload["POW_REACTIVE"] = 1.732*VC_MAG*fixed_current_val*fixed_sin;
